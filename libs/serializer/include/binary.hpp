...
namespace nope
{
  namespace serialization
  {
    ...
    namespace detail
    {
      ...
      template <typename T, bool Enable = std::is_fundamental<typename std::remove_reference<T>::type>::value>
      struct to_binary_impl
      {
        ...
        static void to_binary(buf_t &v, T const &t)
        {
          ...
          apply(t.m_metadata, [&v](auto e) {
            ...
            detail::to_binary_impl<typename std::remove_reference<decltype(e.second)>::type>::to_binary(v, e.second);
          });
        }
      };
    ...
    namespace detail
    {
      ...
      {
        // Function itself
        static void read_bytes(std::uint8_t const *buf, T &e, std::size_t &cursor)
        {
          // Type to read
          T t;
      ...
      {
        // Function itself
        static void read_bytes(std::uint8_t const *buf, std::string &s, std::size_t &cursor)
        {
          // Temporary value
          std::size_t _l;
          // String length
      ...
      struct from_binary_impl
      {
        ...
        static void from_binary(T &t, buf_t const &v, std::size_t &cursor)
        {
          ...
          apply(t.m_metadata, [&v, &cursor](auto e) {
            detail::from_binary_impl<typename std::remove_reference<decltype(e.second)>::type>::from_binary(e.second, v, cursor);
          });
        }
      };
      ...
      {
        // Function itself
        static void from_binary(std::string &s, buf_t const &v, std::size_t &cursor)
        {
          // Read a string from buffer
          detail::read_bytes_impl<std::string>::read_bytes(v.data(), s, cursor);
        }
      };
