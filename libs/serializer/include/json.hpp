    ...
    namespace detail
    {
      ...
      struct to_json_impl
      {
        ...
        static void to_json_(std::stringstream &json, T const &t)
        {
          ...
          json << '{';
          // Loop on each member (e is a std::pair<const char *member_name, T &member>)
      ...
      bool fetch_member_json(T &t, std::stringstream &ss)
      {
        ...
        apply(t.m_metadata, [&name, &ss, &done](auto &e) {
          ...
          if (name == e.first)
          {
            ...
            from_json_impl<typename std::remove_reference<decltype(e.second)>::type>::from_json(e.second, ss);
            done = true;
          }
        });
