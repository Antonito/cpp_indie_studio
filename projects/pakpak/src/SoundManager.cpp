...
void SoundManager::loadSound(const std::string &Filename)
{
  ...
  if (sf_read_short(File, &Samples[0], NbSamples) < NbSamples)
    ...
    nope::log::Log(Debug) << "Reading samples of " << Filename << " has succeed.\n" << "Choose a specific format";
    // Close file
  sf_close(File);
...
{
    nope::log::Log(Debug) << "Setting sound to position {" << x << ", " << y << ", " << z << "}.";
...
{
    nope::log::Log(Debug) << "Set sound to direction ; {" << x << ", " << y << ", " << z << "}.";
...
void SoundManager::getInfoLectureSound() const
{
  ...
  nope::log::Log(Debug) << "\rLecture en cours... " << std::fixed << std::setprecision(2)
            << Seconds << " sec";
}
